        ...
        if(!request()->is('admin/*')){
            ...
            view()->composer('*',function($view)
            {
               if (!Cache::has('recent_posts')) {
                ...
                ->whereHas('category',function ($query){$query->whereStatus(1);})
                ->whereHas('user',function ($query){$query->whereStatus(1);})
               ...
               if (!Cache::has('global_archives')) {
                    ...
                    ->select(DB::raw("Year(created_at) as year"),DB::raw("Month(created_at) as month"))->pluck('year','month')->toArray();
